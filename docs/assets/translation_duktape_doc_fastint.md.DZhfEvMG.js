...
du.d = du.d + 4503599627370496.0;  /* 1&lt;&lt;52 */
return du.d;
</code></pre><p>Zero fastint is simply represented as an IEEE double with all bits zero, which unfortunately needs a separate case.</p><p>In the concrete implementation the fastint_value might include the fastint duk_tval tag and be masked out also for the positive number case.</p><h2 id="future-work" tabindex="-1">Future work <a class="header-anchor" href="#future-work" aria-label="Permalink to &quot;Future work&quot;">​</a></h2><h3 id="fastint-on-platforms-with-no-64-bit-integer-type" tabindex="-1">Fastint on platforms with no 64-bit integer type <a class="header-anchor" href="#fastint-on-platforms-with-no-64-bit-integer-type" aria-label="Permalink to &quot;Fastint on platforms with no 64-bit integer type&quot;">​</a></h3><p>Currently fastint support can only be used if the platform/compiler has support for a 64-bit integer type. This limitation could be removed by implementing alternative fastint fast paths which only relied on 32-bit arithmetic.</p><h3 id="_32-bit-fastint" tabindex="-1">32-bit fastint <a class="header-anchor" href="#_32-bit-fastint" aria-label="Permalink to &quot;32-bit fastint&quot;">​</a></h3><p>It might be worth investigating if a signed or unsigned 32-bit fastint (instead of a signed 48-bit fastint) would be more useful. Fast path arithmetic would certainly be faster.</p><p>The downside would be that some bit operations won&#39;t be possible: to fully support all bit operations both signed and unsigned 32-bit values are needed.</p><h3 id="optimize-upgrade-and-downgrade" tabindex="-1">Optimize upgrade and downgrade <a class="header-anchor" href="#optimize-upgrade-and-downgrade" aria-label="Permalink to &quot;Optimize upgrade and downgrade&quot;">​</a></h3><p>These operations are very important for performance so perhaps inline assembler optimization would be useful for specific platforms, e.g. ARM.</p><p>The current C algorithms can also be optimized further.</p>`,154),s=[o];function r(l,d,p,u,f,c){return a(),t("div",null,s)}const m=e(i,[["render",r]]);export{b as __pageData,m as default};
